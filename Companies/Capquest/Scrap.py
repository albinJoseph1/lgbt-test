...
class Agent(ScrapAgent):
    def __init__(self, chrome, storage = None):
        ...
        self.companyName = "Capquest"
        self.ownerUsername = "capquest"
        self.scrapPageURL = "https://capquest.current-vacancies.com/Careers/Capquest-Vacancy-Search-Page-2120"
    ...
    def loadJobs(self):
        ...
        try:
            ...
            for jobContainer in jobContainers:
                ...
                self.job.setSalary(salary, "£?(\d+(?:,?|\.)\d+)\s*-\s*£?(\d+(?:,?|\.)\d+)|£?(\d+(?:,?|\.)\d+)",is_salary_text=True)
